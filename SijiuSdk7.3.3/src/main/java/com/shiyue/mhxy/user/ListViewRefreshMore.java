/**********************************
 *
 *project name:  ListViewPullPushUpdata      **
 *package name:  com.leo.listview      **  
 *file name:    ListViewRefreshMore.java           **   
 *to do :       TODO
 *time:        2012-12-11   **
 *                                     **
 ***********************************
 ...
 * 
 */
public class ListViewRefreshMore extends ListView implements OnScrollListener,
		OnClickListener {

	private static final String LOGTAG = "ListViewRefreshMore";
	...
	private TextView pull_refresh_time; //上次更新时间
	private TextView pull_time_text; // 上次更新时间标题
	 ...
	 * @param defStyle
	 */
	public ListViewRefreshMore(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 * @param attrs
	 */
	public ListViewRefreshMore(Context context, AttributeSet attrs) {
		super(context, attrs);
		initDragListView(context);
		this.mContext = context;
	}
	...
	public void initHeadView(Context context, String time) {
		...
		mHeadView = LayoutInflater.from(context).inflate(
				getResources().getIdentifier("sjhead", "layout",
						context.getPackageName()), null);
		mArrowImageView = (ImageView) mHeadView.findViewById(getResources()
				.getIdentifier("head_arrowImageView", "id",
						context.getPackageName()));
		mArrowImageView.setMinimumWidth(60);

		mHeadProgressBar = (ProgressBar) mHeadView.findViewById(getResources()
				.getIdentifier("head_progressBar", "id",
						context.getPackageName()));

		mRefreshTextview = (TextView) mHeadView.findViewById(getResources()
				.getIdentifier("head_tipsTextView", "id",
						context.getPackageName()));
		pull_refresh_time = (TextView) mHeadView.findViewById(getResources()
				.getIdentifier("pull_to_refresh_header_time", "id",
						context.getPackageName()));
		
		pull_time_text = (TextView) mHeadView.findViewById(getResources()
				.getIdentifier("pull_to_refresh_last_update_time_text", "id",
						context.getPackageName()));
	...
	private void initLoadMoreView(Context context) {
		mFootView = LayoutInflater.from(context).inflate(
				getResources().getIdentifier("sjfooter_on_net", "layout",
						context.getPackageName()), null);

		mLoadMoreView = mFootView.findViewById(getResources().getIdentifier(
				"load_more_on_net", "id", context.getPackageName()));

		mLoadMoreTextView = (TextView) mFootView.findViewById(getResources()
				.getIdentifier("load_more_text_view", "id",
						context.getPackageName()));

		mLoadingView = (LinearLayout) mFootView.findViewById(getResources()
				.getIdentifier("loading_more_on_net", "id",
						context.getPackageName()));
	...
	private void switchViewState(DListViewState state) {
		...
		case LV_NORMAL: {
			...
			mArrowImageView.setImageResource(mContext.getResources()
					.getIdentifier("arrow", "drawable",
	 ...
	 */
	   public void setLastUpdatedLabel(CharSequence label) {
	        ...
	        pull_time_text.setVisibility(TextUtils.isEmpty(label) ? View.INVISIBLE : View.VISIBLE);
	        pull_refresh_time.setText(label);
	    }
